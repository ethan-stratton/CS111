...
from Grid_Objects import Sand # ?


all_grid_objects = []

# grid we are using, Sand/Rock/Bubble, Coordinates X and Y
def add_object(grid, object_type, x, y):
    ...
    # Make sure the desired grid location is clear
    if grid.get(x,y) is not None:
        return 
    else:
        # Create the object itself, Set the object's own x and y coordinates
        new_thing = object_type(grid, x, y)
        # Store the object in the all_grid_objects list
        all_grid_objects.append(new_thing)
        # Store the object in the grid at the correct x and y coordinates
        grid.set(x,y,new_thing)
...
def remove_object(grid, x, y):
    ...
    # Verify that there is a particle of some kind in the specified position. If not, exit.
    particle = grid.get(x,y)
    if particle is None:
        return
    else:
        # Remove the reference to the object from the grid.
        grid.set(x,y, None)
        # Remove the object from the all_grid_objects list.
        all_grid_objects.remove(particle)
...
def do_whole_grid():
    ...
    # run the move() function of all Bubble objects in the grid (use your all_grid_objects list)
    for particle in all_grid_objects:
        if isinstance(particle, Bubble):
            particle.move()
    # reverse the order of the list with the <list>.reverse() function
    all_grid_objects.reverse()
    # run the move() function of all Sand objects in the grid
    for particle in all_grid_objects:
        if isinstance(particle, Sand):
            particle.move()
    # reverse the order of the list one last time because it will make sorting the list next time just that much easier
    all_grid_objects.reverse()
