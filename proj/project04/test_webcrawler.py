...
def do_valid_args_test(capsys, *args):
    try:
        run_python_script(str(this_folder / 'webcrawler.py'), *args)
    except requests.exceptions.InvalidSchema:
        pass  # ignore any exceptions
    except requests.exceptions.MissingSchema: # for some reason I can't except multiple requests exceptions in one statement
        pass
    except TypeError:
        pass
    captured = capsys.readouterr()
    assert 'invalid arguments' not in captured.out.lower(), f"Your program printed 'invalid arguments' with valid command line arguments {args}."
...
def test_valid_arguments(capsys):
    do_valid_args_test(capsys, '-c', 'asdf', 'args_test_output.png', 'args_test_output.csv')
    do_valid_args_test(capsys, '-p', 'asdf', 'args_test_output.png', 'args_test_output.csv')
    do_valid_args_test(capsys, '-i', 'asdf', 'asdf_', '-g')
    do_valid_args_test(capsys, '-i', 'asdf', 'asdf_', '-s')
    do_valid_args_test(capsys, '-i', 'asdf', 'asdf_', '-m')
    do_valid_args_test(capsys, '-i', 'asdf', 'asdf_', '-f')
